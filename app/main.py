...
def render_task():
    # handle passing in a new file.
    global input_data
    global data_file
    data_file_arg = request.args.get('data_file')
    if data_file_arg is not None:
        data_file_arg = os.path.join(basedir, 'data', data_file_arg)
        if data_file_arg != data_file:
